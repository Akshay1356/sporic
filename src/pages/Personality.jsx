...
export default function Personality() {
  ...
  const leadCourses = useMemo(() => {
    return getAllCourses().filter((c) => c.domain === DOMAINS.LEADERSHIP);
  }, []);

  const subcategories = useMemo(() => {
    const cats = new Set(leadCourses.map((c) => c.category));
    return ['All', ...Array.from(cats)];
  }, [leadCourses]);

  const filteredCourses = useMemo(() => {
    return leadCourses.filter((c) => {
  ...
  }, [leadCourses, selectedSubcategory, searchQuery]);
    ...
    <div className={styles.coursesPage}>
      ...
      <section className={styles.banner}>
        ...
        <div className="container">
          <span className="section-label">Learning Domain • Leadership & Personality</span>
          <h1 className={styles.title}>Leadership & Personality Development</h1>
          <p className={styles.subtitle}>
            Transformational leadership, high-impact executive presence, corporate communication, stress mitigation, and emotional intelligence programs designed for organizational success.
          </p>
        </div>
      </section>
          ...
          <div style={{ marginBottom: '2.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
              ...
              <div style={{ flex: 1, minWidth: '280px', maxWidth: '480px', position: 'relative' }}>
                <input
                  type="text"
                  placeholder="Search Leadership courses by title, ID, or keywords..."
                  value={searchQuery}
              ...
              <div style={{ fontSize: '0.9rem', color: '#0B2A6F', fontWeight: '700', background: '#EFF6FF', padding: '0.6rem 1.1rem', borderRadius: '8px', border: '1px solid #DBEAFE' }}>
                Showing {filteredCourses.length} of {leadCourses.length} Leadership Courses
              </div>
            </div>
