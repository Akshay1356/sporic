...
export default function Technology() {
  ...
  const subcategories = useMemo(() => {
    const cats = new Set(techCourses.map((c) => c.category));
    return ['All', ...Array.from(cats)];
  }, [techCourses]);

  const filteredCourses = useMemo(() => {
    return techCourses.filter((c) => {
  ...
  }, [techCourses, selectedSubcategory, searchQuery]);
    ...
    <div className={styles.coursesPage}>
      ...
      <section className={styles.banner}>
        ...
        <div className="container">
          <span className="section-label">Learning Domain • Technology</span>
          <h1 className={styles.title}>Technology Programs</h1>
          <p className={styles.subtitle}>
            Explore our comprehensive portfolio of {techCourses.length}+ cutting-edge technology courses spanning Industry 4.0, Electric Vehicles, AI & Machine Learning, Advanced Manufacturing, and Cloud Architecture.
          </p>
        </div>
      </section>
          ...
          <div style={{ marginBottom: '2.5rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
              ...
              <div style={{ flex: 1, minWidth: '280px', maxWidth: '480px', position: 'relative' }}>
                <input
                  type="text"
                  placeholder="Search Technology courses by title, ID, or keywords..."
                  value={searchQuery}
              ...
              <div style={{ fontSize: '0.9rem', color: '#0B2A6F', fontWeight: '700', background: '#EFF6FF', padding: '0.6rem 1.1rem', borderRadius: '8px', border: '1px solid #DBEAFE' }}>
                Showing {filteredCourses.length} of {techCourses.length} Technology Courses
              </div>
            </div>
